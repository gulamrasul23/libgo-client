...
const AddBook = () => {
    ...
    }, [updateBook, bookReset]);

    const handleStatus = async (data) => {
        setLoading(true);
        try {
            const res = await axiosSecure.patch(`/books/update-book/${id}`, { status: data.status });
    ...
    const handleUpdateBook = async (data) => {
        ...
        if (bookImg) {
            ...
            try {
                ...
                bookImageUrl = res.data.data.url;
            } catch (error) {
                Swal.fire({
                    title: "Something Went Wrong...!",
                    text: `${error.message}`,
        ...
        <div className="min-h-[calc(100vh-64px)] bg-base-300/50 flex items-center p-2 justify-center ">
            ...
            <div className="w-full max-w-4xl bg-base-100 rounded-xl shadow-lg overflow-hidden border border-gray-100">
                <div className="bg-secondary px-4 py-3 mb-4 gap-4 sm:gap-1 flex flex-col sm:flex-row items-center rounded-xl  justify-between ">
                    ...
                    <form onSubmit={handleBookStatusUpdate(handleStatus)} className="flex gap-1 items-center">
                        <div className="">
                            <label className="pr-2 flex items-center text-white font-medium"><input type="radio" {...bookStatusUpdate('status')} value="Published" className="radio radio-primary mr-1" />Published</label>
                        </div>
                        <div>
                            <label className="pr-2 flex items-center text-white font-medium"><input type="radio" {...bookStatusUpdate('status')} value="Unpublished" className="radio radio-primary mr-1" />Unpublished</label>
                        </div>
                        <button
                            type="submit"
                ...
                <form onSubmit={handleBookInfoUpdate(handleUpdateBook)} className="p-8 space-y-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div className="form-control">
                            ...
                            <input
                                type="number"
                                {...bookInfoUpdate("price", { required: true })}
                    ...
                    <div className="form-control">
                        ...
                        <textarea
                            {...bookInfoUpdate("description", { required: true, minLength: 20 })}
                            rows="4"
